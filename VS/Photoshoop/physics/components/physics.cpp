...
void PhysComp::respondToCollision(PhysComp &other) {
    ...
    double clipping = radius + other.radius - (ACCOUNT_FOR_SKIPPED_MOVEMENT ? 2. : 1.) * axis.length();
...
abel::gui::Color MagneticPhysComp::getTypeColor() const noexcept {
    static constexpr double MAX_CHARGE = 0.05;
    ...
    return base * (float)std::abs(charge) / (float)MAX_CHARGE;
}
