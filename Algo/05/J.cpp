...
struct Mask {
    ...
    constexpr void setBit(unsigned idx) {
        if constexpr (Value) {
            raw |= 1u << idx;
        } else {
            raw &= ~(1u << idx);
    ...
    constexpr void bit(unsigned idx, bool value) {
        if (value) {
            setBit<true>(idx);
        } else {
            setBit<false>(idx);
    ...
    constexpr bool next(unsigned bits) {
        return (++raw) < (1u << bits);
    }
};
...
static double solve() {
    ...
    for (auto &cur : points) {
        std::cin >> cur.pos >> cur.angle;

        DBG(">> %lg - %u", cur.pos.x(), l);
        cur.pos.x() -= l;
        DBG(">> %lg", cur.pos.x());
        // We want to keep all positions above y=0
        cur.pos.y() = std::abs(cur.pos.y());
    }
    ...
    do {
        for (unsigned i = 0; i < n; ++i) {
            ...
            DBG("> %u %lg", i, points[i].pos.x());
            Vec2d ray = Vec2d{dp[mask.raw], 0} - points[i].pos;

            DBG("%lg %lg", ray.x(), ray.y());
            ray.rotateDegrees(points[i].angle);
            DBG("%lg %lg", ray.x(), ray.y());
            ...
            ray *= -points[i].pos.y() / ray.y();
            ray += points[i].pos;

            assert(sgnDbl(ray.y()) == 0);

            Mask next{mask};
            next.setBit<true>(i);

            // DBG("%lg %lg", dp[i], ray.x());

            dp[next.raw] = std::max(dp[next.raw], std::max(dp[mask.raw], ray.x()));
        }
    } while (mask.next(n));

    return std::min(dp[(1 << n) - 1], (double)r);
}
