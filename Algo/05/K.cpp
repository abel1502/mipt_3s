...
struct Edge {
    ...
    constexpr decltype(auto) operator<=>(const Edge &other) const noexcept {
        return other.magnitude <=> magnitude;  // Inverse
    }
};


static num_t solve() {
    using bitset = std::bitset<3010>;

    unsigned n = 0;
    std::cin >> n;

    std::vector<Vec2i> centers(n);
    std::vector<Edge> edges{};
    std::vector<bitset> used(n, bitset(0));
    ...
    for (unsigned i = 0; i < n; ++i) {
        for (unsigned j = i + 1; j < n; ++j) {
            edges.push_back(Edge{i, j, (centers[i] - centers[j]).magnitude()});
        }
    }
    ...
    for (const auto &edge : edges) {
        if ((used[edge.from] & used[edge.to]).any()) {
            return edge.magnitude;
        }

        used[edge.from][edge.to] = used[edge.to][edge.from] = true;
    }
