...
static void solve() {
    ...
    Vec2i ray1 =  norm1.orthogonalized();
    Vec2i ray2 = -norm2.orthogonalized();

    std::cout << -ray1 << "\n";
    std::cout <<  ray2 << "\n";

    num_t det = (norm1 & norm2);
    if (det) {
        std::cout << Vec2d{ray2 * c1 + ray1 * c2} / det;
    } else {
        Vec2d dir = Vec2d{ray1}.normalized();

        double dist = std::abs((double)c1 / (Vec2d{norm1} & dir) -
                               (double)c2 / (Vec2d{norm2} & dir));
