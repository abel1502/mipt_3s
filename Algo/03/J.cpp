...
public:
    struct Suffix {
        ...
        unsigned getLength(const SuffixArray &array) const noexcept {
            return array.str.size() - 1 - start;
        }
    };
    ...
    std::string_view kthSubstr(unsigned k) const {
        ...
        std::stack<Block> blockEnds{};
        blockEnds.push(Block{suffixes.size(), 0});

        while (k > 0 && idx < suffixes.size()) {
            while (idx >= blockEnds.top().end) {
                blockEnds.pop();
            }

            length = blockEnds.top().length + 1;

            if (length <= suffixes[idx].getLength(*this)) {
                unsigned remaining = suffixes[idx].getLength(*this) - length;
                if (k < remaining) {  // <=?
                    length += k;
                    k = 0;
                    break;
            ...
            unsigned blockEnd = findBlockEnd(idx, length);
            if (k < blockEnd - idx) {  // <=?
                // idx += k;  // Does nothing
                k = 0;
                break;
            ...
            blockEnds.push(Block{blockEnd, length});
        }
