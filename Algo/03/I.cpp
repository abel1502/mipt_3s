...
public:
    ...
    struct Node {
        ...
        void setDfsIdxs(SuffixTree &tree, idx_t &idx) {
            dfsIdx = idx++;

            for (auto child : children) {
                tree.nodes[child.second].setDfsIdxs(tree, idx);
            }
        }

        void output(SuffixTree &tree) {
            if (!isRoot()) {
                printf("%u %u %u\n", tree.nodes[parent].dfsIdx, strL, strR);
            }

            for (auto child : children) {
                tree.nodes[child.second].output(tree);
            }
        }
