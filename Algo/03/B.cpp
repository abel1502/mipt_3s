...
public:
    ...
    struct Node {
        ...
        unsigned findKthSubstr(const SuffixTree &tree, unsigned long long &k, unsigned &end) const noexcept {
            unsigned length = 0;
            ...
            for (auto child : children) {
                length = tree.nodes[child.second].findKthSubstr(tree, k, end);
    ...
    std::string_view findKthSubstr(unsigned long long k) const noexcept {
        assert(nodes.size() > 0);

        unsigned strR = 0, length = 0;
        length = nodes[0].findKthSubstr(*this, k, strR);
