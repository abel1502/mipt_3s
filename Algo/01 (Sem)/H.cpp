...
static void solve() {
    ...
    std::vector<Vec2i> convexHull = getConvexHull(points);

    unsigned left  = 0,
             right = 0;

    {
        Vec2i  leftVec = convexHull.front();
        Vec2i rightVec = convexHull.front();

        for (unsigned i = 0; i < convexHull.size(); ++i) {
            if (convexHull[i].x() < leftVec.x()) {
                left = i;
                leftVec = convexHull[i];
            }

            if (convexHull[i].x() > rightVec.x()) {
                right = i;
                rightVec = convexHull[i];
            }
        }
    }

    unsigned  leftBestIdx = convexHull[ left].idx,
             rightBestIdx = convexHull[right].idx;

    double diameter = (convexHull[left] - convexHull[right]).magnitude();

    for (unsigned _i = 0; _i < convexHull.size() * 2; ++_i) {
        unsigned nextLeft  = (left  + 1) % convexHull.size(),
                 nextRight = (right + 1) % convexHull.size();

        if (((convexHull[nextLeft ] - convexHull[left ]) &
             (convexHull[nextRight] - convexHull[right])) < 0) {
            left = nextLeft;
        } else {
            right = nextRight;
        }

        double curDiam = (convexHull[left] - convexHull[right]).magnitude();
        if (curDiam > diameter) {
            diameter = curDiam;
             leftBestIdx = convexHull[ left].idx;
            rightBestIdx = convexHull[right].idx;
        }
    }

    std::cout << std::min(leftBestIdx, rightBestIdx) << " " << std::max(leftBestIdx, rightBestIdx) << "\n";
}
